            ...
            loop {
                ...
                select! {
                    Ok(n) = device.read(&mut buffer) => {
                        ...
                        println!("ipstack comming from tun  {:?}",packet.network_tuple());
                    ...
                    Some(packet) = pkt_receiver.recv() => {
                        //println!("sending to tun {:?}",packet.reverse_network_tuple());
                        // if packet.network_tuple().dst.to_string() == "6.6.6.6:6"{
                        //     println!("sending to tun {:?}",packet.reverse_network_tuple());
                        // }
                        if packet.ttl() == 0{
